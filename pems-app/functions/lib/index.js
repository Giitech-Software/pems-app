...
function toPaymentMethod(channel) {
...
async function verifyBearerToken(authorizationHeader) {
...
async function assertPaymentRequestMatchesTenant(paymentRequest, uid) {
    ...
    const fieldsMatch = paymentRequest.ownerId === tenant.ownerId &&
...
export const paystackWebhook = onRequest({
    ...
    try {
        await db.runTransaction(async (transaction) => {
            ...
            const paymentRequest = requestSnapshot.data();
            if (paymentRequest.status === "paid") {
                return;
            }
